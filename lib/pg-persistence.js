...
class PgPersistence {
  ...
  async saveSearchToDatabase(searchId, userEmail, jsonBody) {
    const userId = await this.findUserIdByEmail(userEmail);
    if (!userId) return false;

    const sql = 'INSERT INTO searches (id, user_id, json_data)' +
                ' VALUES ($1, $2, $3)';
    
    await dbQuery(sql, searchId, userId, JSON.stringify(jsonBody));              
    return true;
  }
}
